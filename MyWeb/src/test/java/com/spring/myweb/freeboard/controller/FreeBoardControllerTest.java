...
//	private FreeBoardController controller;
	...
	@BeforeEach //테스트 시작 시 항상 먼저 구동되는 기능들을 선언
	public void setup() {
		//가상 구조를 세팅
...
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	@Test
	@DisplayName("3번 글 상세 보기 요청을 넣으면 컨트롤러는 DB에서 "
			+ "가지고 온 글 객체를 model에 담아서 jsp로 이동시킬 것이다.")
	...
	@Test
	@DisplayName("3번글의 제목과 내용을 수정하는 요청을 post방식으로 "
			+ "전송하면 수정이 진행되고, 수정된 글의 상세보기 페이지로 이동할 것이다.")
	// /freeboard/modify -> post
	void testModify() throws Exception {
		String bno = "3";
		String viewPage = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/update")
				.param("bno", bno)
				.param("title", "ddd")
				.param("content", "fff"))
				.andReturn().getModelAndView().getViewName();
