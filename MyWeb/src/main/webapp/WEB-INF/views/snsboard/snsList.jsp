...
<body>
	...
	<script>
		...
		function regist() {
			...
			// 세션에서 현제 로그인 중인 사용자의 정보(아이디)를 얻어오자,
			...
			// .을 제거한 확장자만 얻은 후 그것을 소문자로 일괄 변경
			...
			if(file !== 'jpg' && file !== 'png' && file !== 'jpeg' && file !== 'bmp') {
				...
				return;
			} else if(userId === '') { // 세션데이터가 없음 - 로그인 안한 상태
				alert('로그인이 필요합니다');
				return;
			}
			/*
			...
			대부분의 경우에은 form 태그를 이용해서 전송하는 방식이 더 간편하고 더 자주 사용됩니다.
			*/
			...
			// 한 화면에 파일 업로드 버튼이 여러 개일 경우 요소의 인덱스를 지목해서 가져올수있음
			...
			// FileList는 유사 배열이기 떄문에 인덱스를 이용해서 여러 파일 중 하나를 지목할 수 있습니다.
			// 우리는 지금 multiple이 없기 떄문에 [0]이라고 작성하면 파일 정보를 얻을 수 있습니다.
			...
			// FormData 객체를 보낼 떄는 따로 geaders 설정을 진행하지 않습니다.
		...
		function getList(page, reset) {
			...
			.then(list => {
				...
				if(!reset) {
				document.getElementById('contentDiv').insertAdjacentHTML('beforebegin', str);
				} else {
		...
		사용자가 현재 우치를 알기가 힘들고, 원하는 페이지에 도달하기 위해
		...
		*/
		window.onscroll = function() {
			...
			문서(컨텐츠)의 높이와 비교해서 같아졌따면 로직을 수행.
			...
			*/
			if(window.innerHeight + window.scrollY >= document.body.scrollHeight) {
				...
				게시글을 한 번에 몇 개씩 불러 올지는 pageVO를 조정하시면 됩니다.
				*/
				console.log('페이징 발동!');
				getList(++page, false);	
			} else {
