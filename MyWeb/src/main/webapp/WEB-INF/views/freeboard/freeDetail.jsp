...
<script>
    window.onload = function () {
		...
		//더보기 버튼 처리(클릭 시 전역 변수 pag에 +1 한 값을 요청)
		...
		그러다 보니 이벤트가 등록되는 시전보다 fetch 함수의 실행이 먼저 끝날 것이라는
		...
		*/
		...
		document.getElementById('replyList').addEventListener('click', e => {
			...
			//1. 이벤트가 발생한 target이 a태그가 아니라면 이벤트 중지.
		...
		//수정 처리 함수. (수정 모달을 열어서 수정 내용을 작성 후 수정 버튼을 클릭햇을 )
		...
		document.getElementById('modalDelBtn').onclick = () => {
			...
			3. 서버에서는 요청을 받아서 비밀번호를 확인하고, 비밀번호가 ㅁㅈ으면
			 삭제를 진행하시면 됩니다.
			4. 만약 비밀번호가 틀렸다면, 문자열을 반환해서
			'비밀번호가 틀렸습니다.' 경고창 띄우세요.

			5. 삭제 완료되면 모달 닫고 목록 요청 다시 보내세요. (reset의 여부를 잘 판단)
			*/
