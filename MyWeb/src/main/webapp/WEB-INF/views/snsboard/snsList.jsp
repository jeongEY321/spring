...
<body>
	...
	<script>
					...
					for (vo of list) {
						str +=
							...
							<div class="title">
								...
								<a id="download" href="${pageContext.request.contextPath}/download/` + vo.fileLoca+ `/` + vo.fileName + `">이미지 다운로드</a>
							</div>
						</div>
						<div class="content-inner">
							<!--내용영역-->
		...
		document.getElementById('contentDiv').addEventListener('click', e => {
			...
			if(!e.target.matches('.image-inner img') && !e.target.matches('.link-inner #comment') && !e.target.matches('.link-inner #download')) {
        ...
        // 일치하지 않는다면 문자열 "noAuth" 리턴, 삭제 완료하면 "seccess" 리턴
        ...
        document.getElementById('contentDiv').addEventListener('click', e => {
			...
			fetch('${pageContext.request.contextPath}/snsboard/' + bno, {method: 'delete'})
				.then(res => res.text())
				.then(result => {
					if(result === 'noAuth'){
						alert('권한이 없습니다!');
